...
describe("PledgePool", function (){
    ...
    beforeEach(async ()=>{
        ...
        spAddress = await spToken.deploy("spBUSD_1","spBUSD_1", mockMultiSignature.target);
        await spAddress.waitForDeployment();
        const jpToken = await ethers.getContractFactory("DebtToken");
        jpAddress = await jpToken.deploy("jpBTC_1", "jpBTC_1", mockMultiSignature.target);
        ...
        // approve multi-signature for DebtToken admin operations
        await mockMultiSignature.approve(minter.address, spAddress.target, 0);
        await mockMultiSignature.approve(minter.address, jpAddress.target, 0);
    });
    ...
    async function initCreatePoolInfo(pledgeAddress, minter, time0, time1){
        ...
        let settleTime = toBI(startTime) + toBI(time0);
        show({settleTime});
        let endTime = settleTime + toBI(time1);
        show({endTime});
    ...
    it("check if mint right", async function() {
        ...
        expect(await spAddress.totalSupply()).to.equal(100000000n);
        expect(await spAddress.balanceOf(alice.address)).to.equal(100000000n);
        expect(await jpAddress.totalSupply()).to.equal(100000000n);
        expect(await jpAddress.balanceOf(alice.address)).to.equal(100000000n);
    ...
    it ("deposit lend after create pool info, pool state is match", async function (){
        ...
        show({data});
        expect(data[0]).to.be.equal(1000000000000000000000n);
        // increase
        await increase(1000);
    ...
    it ("pause check", async function (){
        ...
        expect(num[0]).to.be.equal(1000000000000000000000n);
        // paused
    ...
    it("pool state check", async function (){
        ...
        await busdAddress.connect(minter).approve(pledgeAddress.target, BigInt(1000*1e18));
        await pledgeAddress.connect(minter).depositLend(0, BigInt(1000*1e18));
        const target = (await latestBlock()) + 10;
        await advanceBlockTo(target);
    ...
    it("claim spToken or jpToken, pool state is execution", async function (){
        ...
        expect(await pledgeAddress.getPoolState(0)).to.equal(1);
        ...
        expect(await spAddress.balanceOf(minter.address)).to.equal(250000000000000000000n);
        expect(await jpAddress.balanceOf(minter.address)).to.equal(500000000000000000000n);
    });
    ...
    it ("lend burn sp token and borrow burn jp token, pool is finish", async function (){
        ...
        await pledgeAddress.connect(minter).settle(0);
        show(await pledgeAddress.getPoolState(0));
        expect(await pledgeAddress.getPoolState(0)).to.equal(1n);
        // claim sp_token or jp_token
        let poolDataInfo = await pledgeAddress.poolDataInfo(0);
        show({poolDataInfo});
        // sp and jp add minter
        ...
        await increase(3000);
        // add liquidate
        let deadLineAddLiquidate = (await latest()) + 1000;
        ...
        expect(await pledgeAddress.getPoolState(0)).to.equal(2n);
